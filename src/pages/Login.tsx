...
const Login: React.FC = () => {
  const { login, authMode, ssoFailed } = useAuth();
  ...
  if (authMode === 'sso') {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
          <div className="glass-panel p-8 rounded-2xl max-w-md w-full text-center">
            <h2 className="text-2xl font-bold text-destructive mb-4">SSO Failed</h2>
            <p className="text-muted-foreground">Single Sign-On authentication could not be completed.</p>
          </div>
        </div>
      );
    }
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="animate-pulse text-primary">Authenticating via SSO...</div>
      </div>
    );
  }
